const Reports = (() => {
  const REPORTS = [
    ...
    { id:'ledger', name:'Personal Ledger Report', desc:'Income, expenses, borrowed money, and loans given', icon:'book' },
  ];
  ...
  function buildRows(id){
    ...
    switch(id){
      ...
      case 'ledger':
        return Storage_DB.data.personalLedger.entries.slice().sort((a,b)=>new Date(b.date)-new Date(a.date)).map(e=>({
          Date:Utils.fmtDate(e.date), Type:e.type, Person:e.person||'', Place:e.place||'',
          WhatFor:e.description||'', Amount:e.amount, Status: (e.type==='borrowed'||e.type==='loan') ? e.status : '—',
          SettledDate: e.settledDate ? Utils.fmtDate(e.settledDate) : '', Notes:e.notes||''
        }));
      default: return [];
    }
  }
